...
| Weekly bundle compose, bytes (S3/R2, 2026-09-16) | 1 HEAD per source → 1 CreateMultipartUpload → ⌈total/part_size⌉ parts (UploadPartCopy inside one source, UploadPart for the boundary part) → 1 Complete; `part_size = clamp(⌈total/1024⌉, 5 MiB, 32 MiB)`, so ≤ ~1024 parts and only one part (≤32 MiB) streams through the process. R2 requires every non-trailing part to be the same length (#221) | 4 + parts (2 sources) | `s3.rs::compose` |
...
| Repository listing (`/api/v1/owners*`, `/services/api/owners*`, maintainer passes) | 0 within `LIST_TTL` (30 s, per instance); else delimited `repos/` → (delimited `repos/<o>/` ∥ owners) → (HEAD `manifest.pb` ∥ repos): 3 rounds | 1 + owners + repos | `registry.rs::list` |
