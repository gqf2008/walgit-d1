...
pub fn info(cfg: &walgit_config::Config) -> InstanceInfo {
    ...
    let instance = env("WALGIT_INSTANCE_ID").map_or_else(|| std::process::id().to_string(), |i| {
            i.chars()
                .rev()
                .take(6)
                .collect::<String>()
    ...
    let shape = match kind {
        "ssd" => gce_machine_type().map_or_else(|| format!("{cpus} vCPU · {}", gib(memory_bytes)), |m| format!("{m} · {cpus} vCPU · {}", gib(memory_bytes))),
