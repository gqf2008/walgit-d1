...
async fn cli_full_collab_flow_against_a_real_server() -> TestResult {
    ...
    assert!(
        text.contains("add thing"),
        "titled thread renders by title: {text}"
    );
...
async fn watch_reports_new_collab_entries_via_callback() -> TestResult {
    ...
    assert!(
        principal_file.exists(),
        "cold start delivered the principal record"
    );
    ...
    assert!(
        comment_file.exists(),
        "new comment delivered: {}",
        comment_file.display()
    );
    let comment_v: serde_json::Value =
        serde_json::from_str(&std::fs::read_to_string(&comment_file)?)?;
...
async fn board_projection_is_byte_identical_across_clients_and_moves_with_status() -> TestResult {
    ...
    assert_eq!(
        column_of("review"),
        vec!["t1"],
        "status entry put t1 on review: {board}"
    );
    ...
    assert_eq!(
        arr.len(),
        2,
        "issue + the moving status entry: {moved_thread}"
    );
    assert!(
        arr.iter()
            .all(|e| e["verified"] == serde_json::Value::Bool(true)),
    ...
    let moved_server = reqwest::get(format!("{base}/o/r/api/collab/board"))
        .await?
        .bytes()
        .await?;
    assert_eq!(
        moved_cli.as_bytes(),
        &moved_server[..],
        "post-move: still byte-identical"
    );
    ...
    assert_eq!(
        review_sorted,
        vec!["t1", "t2"],
        "the move put t2 on review: {moved_board}"
    );
    assert_eq!(
        review.first(),
        Some(&"t2".to_string()),
        "newest activity first: {moved_board}"
    );
...
async fn gc_fold_keeps_aggregation_byte_identical_and_the_tail_stays_live() -> TestResult {
    ...
    let status = oid_of(&run(&[
        ...
        &comment,
        "--body",
        r#"{"status":"needs-review","owner":"svc-a"}"#,
        "--key",
        alice_k,
        "--push",
        "origin",
    ...
    git_in(
        a.path(),
        &["push", "-q", "origin", "refs/collab/inbox/carol/driveby1"],
    )?;
    ...
    run(&[
        ...
        "--body",
        &format!(
            r#"{{"task":"test","ref":"refs/heads/main","commit":"c0ffee","attempt":1,"claim":"{claim}","conclusion":"success","exit_code":0,"duration_ms":12,"log_summary":"ok","log_sha256":""}}"#
        ),
        "--key",
        bob_k,
        "--push",
    ...
    assert_eq!(
        collab_refs(b.path())?
            .iter()
            .filter(|l| l.contains("refs/collab/inbox/"))
            .count(),
        11,
    ...
    let pre_board_srv = reqwest::get(format!("{base}/o/r/api/collab/board"))
        .await?
        .bytes()
        .await?;
    assert_eq!(
        pre_board_cli.as_bytes(),
        &pre_board_srv[..],
        "pre-fold: CLI == server"
    );
    let pre_thread_cli = run(&["collab", "thread", "t1", "--repo", repo_b])?;
    let pre_thread_srv = reqwest::get(format!("{base}/o/r/api/collab/threads/t1"))
        .await?
        .bytes()
        .await?;
    let pre_report_srv = reqwest::get(format!("{base}/o/r/api/collab/report"))
        .await?
        .bytes()
        .await?;
    ...
    assert!(
        after
            .iter()
            .any(|l| l.contains("refs/collab/meta/snapshot")),
    ...
    assert_eq!(
        post_board_cli, pre_board_cli,
        "fold must not move the board"
    );
    let post_thread_cli = run(&["collab", "thread", "t1", "--repo", repo_c])?;
    assert_eq!(
        post_thread_cli, pre_thread_cli,
        "fold must not move the thread view"
    );
    let post_board_srv = reqwest::get(format!("{base}/o/r/api/collab/board"))
        .await?
        .bytes()
        .await?;
    assert_eq!(
        &post_board_srv[..],
        &pre_board_srv[..],
        "server board identical across the fold"
    );
    assert_eq!(
        post_board_cli.as_bytes(),
        &post_board_srv[..],
        "post-fold: CLI == server"
    );
    let post_thread_srv = reqwest::get(format!("{base}/o/r/api/collab/threads/t1"))
        .await?
        .bytes()
        .await?;
    assert_eq!(
        &post_thread_srv[..],
        &pre_thread_srv[..],
        "server thread identical across the fold"
    );
    let post_report_srv = reqwest::get(format!("{base}/o/r/api/collab/report"))
        .await?
        .bytes()
        .await?;
    assert_eq!(
        &post_report_srv[..],
        &pre_report_srv[..],
        "server report identical across the fold"
    );
    ...
    let parsed: Vec<_> = snap
        ...
        .collect();
    let verified_count = parsed
        .iter()
        .filter(|e| e.is_verified(&principals_c))
        .count();
    assert_eq!(
        verified_count, 9,
        "9 verified, carol + wrong-key unverified"
    );
    let carol = parsed
        .iter()
        .find(|e| e.entry.actor == "carol")
        .expect("carol's entry folded");
    ...
    assert!(
        !wrong_key.is_verified(&principals_c),
        "wrong-key signature stays unverified"
    );
    ...
    assert_eq!(
        arr[6]["entry"]["parent"], carol_oid,
        "chained onto a folded oid"
    );
    assert!(
        arr.iter()
            .all(|e| e["verified"] == serde_json::Value::Bool(true)
    ...
    let tail_thread_srv = reqwest::get(format!("{base}/o/r/api/collab/threads/t1"))
        .await?
        .bytes()
        .await?;
    let tail_board_cli = run(&["collab", "board", "--repo", repo_c, "--format", "json"])?;
    let tail_board_srv = reqwest::get(format!("{base}/o/r/api/collab/board"))
        .await?
        .bytes()
        .await?;
    assert_eq!(
        tail_board_cli.as_bytes(),
        &tail_board_srv[..],
        "post-append: CLI == server"
    );
    ...
    assert_eq!(
        snap2.entries.len(),
        12,
        "the second fold accumulated the tail"
    );
    ...
    assert_eq!(
        final_thread_cli, tail_thread_cli,
        "second fold: CLI thread stable"
    );
    let final_thread_srv = reqwest::get(format!("{base}/o/r/api/collab/threads/t1"))
        .await?
        .bytes()
        .await?;
    assert_eq!(
        &final_thread_srv[..],
        &tail_thread_srv[..],
        "second fold: server thread stable"
    );
    Ok(())
}
...
async fn a_stale_fold_baseline_loses_the_lease_and_the_crash_resume_prunes_deduped() -> TestResult {
    ...
    assert_eq!(
        remote_snapshot(c.path())?,
        s1,
        "the retry did not move the snapshot"
    );
    ...
    git_in(
        b.path(),
        &["update-ref", "refs/collab/inbox/mallory/crashed", &issue1],
    )?;
    git_in(
        b.path(),
        &["push", "-q", "origin", "refs/collab/inbox/mallory/crashed"],
    )?;
    ...
    assert_eq!(
        remote_inbox(b.path())?,
        0,
        "the crash-resumed duplicate was pruned"
    );
    assert_eq!(
        remote_inbox(c.path())?,
        0,
        "c's stale copies never reappear remotely"
    );
    fetch_all(b.path())?;
    ...
    assert_eq!(
        ...
        "the planted copy never displaces the legitimate record"
    );
    assert!(
...
async fn sha256_repo_fold_preserves_the_thread() -> TestResult {
    ...
    let snapshot =
        walgit_wal::collab::parse_snapshot(snapshot.as_bytes()).expect("snapshot parses");
