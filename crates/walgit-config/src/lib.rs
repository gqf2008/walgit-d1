    ...
    pub fn tls_hostnames(&self) -> Vec<String> {
        ...
        if let Some(u) = &self.server.public_url {
            let host = u
                ...
                .trim_start_matches('[');
            let host = host
                .rsplit_once(']').map_or_else(|| host.split(':').next().unwrap_or(host), |(h, _)| h);
...
fn rewrite_origin_port(origin: &str, port: u16) -> String {
    ...
    let host = if rest.starts_with('[') {
        rest.split_once(']').map_or_else(|| rest.to_string(), |(h, _)| format!("{h}]"))
    } else {
...
mod tests {
    ...
    fn env_override_text_that_looks_like_a_toml_scalar_applies() {
        ...
        assert_eq!(c.maintenance.follow_interval, std::time::Duration::ZERO);
